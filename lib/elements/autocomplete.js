...
class AutocompletePrompt extends Prompt {
  ...
  moveSelect(i) {
    this.select = i;
    if (this.suggestions[this.page].length > 0) this.value = getVal(this.suggestions[this.page], i);
  ...
  render() {
    ...
    let prompt = `${style.symbol(this.done, this.aborted)} ${color.bold(this.msg)} ${style.delimiter(this.completing)} `;
