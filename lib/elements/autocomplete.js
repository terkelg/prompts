 ...
 * @param {String} [opts.fallback] Fallback message - defaults to default value
...
class AutocompletePrompt extends Prompt {
  constructor(opts={}) {
    ...
    this.fallback = opts.fallback || opts.initial ? `› ${getVal(this.choices, this.initial)}` : `› no matches found`;
    this.suggestions = [];
    this.input = '';
  ...
  moveCursor(i) {
    this.cursor = i;
    if (this.suggestions.length > 0) this.value = getVal(this.suggestions, i) || null;
    else this.value = this.initial ? getVal(this.choices, this.initial) : null;
    this.fire();
  }
