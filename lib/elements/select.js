            ...
            .map((v, i) => {
              ...
              if (v.disabled) {
                ...
                if (v.description && this.cursor === i) {
                  desc = ` - ${v.description}`;
                  if (prefix.length + title.length + 3 + v.description.length >= this.out.columns
                    ...
                    desc = '\n' + v.description
                        ...
                        .map(line => line
                            .split(/\s+/g)
                            .reduce((arr, w) => {
                              if (w.length + 3 > this.out.columns || arr[arr.length - 1].length + w.length + 1 < this.out.columns)
                                arr[arr.length - 1] += ` ${w}`;
                              else arr.push(`   ${w}`);
                              return arr;
                            }, [ '   ' ])
