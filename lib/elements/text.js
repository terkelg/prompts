...
class TextPrompt extends Prompt {
  ...
  render() {
    ...
    let rowCount = lines(` `.repeat(outputLength)) - 1;
    let curRowLength = outputLength - (this.out.columns * rowCount);
    if (curRowLength === this.out.columns) {
      rowCount += 1;
      curRowLength = 0;
    }
    let cursorGrossPos = this.cursor - this.value.length + outputLength;
    let cursorRowCnt = lines(` `.repeat(cursorGrossPos)) - 1;
    let cursorHorzPos = cursorGrossPos - (this.out.columns * cursorRowCnt);
    if (cursorHorzPos === this.out.columns) {
      cursorRowCnt += 1;
      cursorHorzPos = 0;
    }
    ...
    this.out.write(
      (this.prevRowCount > 0 ? cursor.prevLine(this.prevRowCount) : ``) +
      cursor.to(0) +
      this.outputPrompt +
      (this.placeholder ? cursor.save : ``) +
      this.outputText +
      (!this.placeholder ? cursor.save : ``) +
      (curRowLength === 0 && rowCount > 0 ? `\n` : ``) + erase.lineEnd +
      (this.postMessage ?
        `\n${outputPostMessage}` +
        erase.down() +
    ...
    this.prevRowCount = rowCount - cursorRowOffset;
  }
}
