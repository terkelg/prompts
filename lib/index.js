...
async function prompt(questions=[], { onSubmit=noop, onCancel=noop }={}) {
  ...
  for (question of questions) {
    ...
    if (typeof question.message !== 'string') {
      throw new Error(translate('messageRequired'));
    }
    ...
    if (prompts[type] === void 0) {
      throw new Error(util.format(translate('typeUndefined'), type));
    }
...
function localize(customMessages) {
  translate.localize(customMessages);
}

module.exports = Object.assign(prompt, { prompt, prompts, inject, localize });
